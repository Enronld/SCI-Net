...
from transformers import AutoTokenizer, AlbertTokenizer, AlbertConfig
...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        for epoch in range(args.epochs):
            ...
            if not args.final_eval or (epoch == args.epochs - 1):  
                avg_score = self._eval(model, validation_dataset, input_reader, epoch + 1, updates_epoch)
        ...
        pattern = re.compile(r'label = train_(.*?)\n', re.S)
        dataset_name = [i for i in pattern.finditer(train_content)][0].group(1)


        
        with open("./configs/example_eval_" + dataset_name + "-test.conf", 'r', encoding="utf-8") as file:
            eval_content = file.read()

        old_model_path = r'model_path = (.*?)\n'
        new_model_path = 'model_path = ' + self._save_path + "/final_model" + "\n"

        modified_content = re.sub(old_model_path, new_model_path, eval_content)

        old_tokenizer_path = r'tokenizer_path = (.*?)\n'
        new_tokenizer_path = 'tokenizer_path = ' + self._save_path + "/final_model" + "\n"

        modified_content = re.sub(old_tokenizer_path, new_tokenizer_path, modified_content)

        
        with open("./configs/example_eval_" + dataset_name + "-test.conf", 'w', encoding="utf-8") as file:
        ...
        record_file = "./" + dataset_name + "_shell_train.txt"

        
        if not os.path.exists(record_file):
            
            with open(record_file, "w", encoding='utf-8') as txt_file:
                txt_file.write("train_batch_size,epochs,lr,lr_warmup,weight_decay,max_grad_norm,rel_filter_threshold,size_embedding,head,prop_drop,max_span_size,seed,log_path(model_path)\n")
              

        with open(record_file, "a", encoding='utf-8') as txt_file:
            txt_file.write(str(args.train_batch_size) + "," + str(args.epochs) + "," + str(args.lr) + "," + str(args.lr_warmup) + "," +
                                str(args.weight_decay) + "," + str(args.max_grad_norm) + "," + str(args.rel_filter_threshold) + "," + str(args.size_embedding) + "," + 
                                str(args.head) + "," + str(args.prop_drop) + "," + str(args.max_span_size) + "," + str(args.seed) + "," + self._log_path + "\n")
    ...
    def eval(self, dataset_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        pattern = re.compile(r'label = eval_(.*?)\n', re.S)
        dataset_name = [i for i in pattern.finditer(eval_content)][0].group(1)

        
        record_file = "./" + dataset_name + "_shell.txt"



        
        with open(self._log_path + "/" + "eval_test.csv", 'r', newline='', encoding="utf-8") as csv_file:
            csv_reader = csv.reader(csv_file)

            if not os.path.exists(record_file):
                
                with open(record_file, "w", encoding='utf-8') as txt_file:
                    txt_file.write("eval_batch_size,rel_filter_threshold,size_embedding,head,prop_drop,max_span_size,seed,model_path,log_path, ,")
                    for i, csv_head in enumerate(csv_reader):
                        if i == 0:
                            txt_file.write(','.join(csv_head[0].split(";")) + "\n")           

            
            with open(record_file, "a", encoding='utf-8') as txt_file:
                txt_file.write(str(args.eval_batch_size) + "," + str(args.rel_filter_threshold) + "," + str(args.size_embedding) + "," + 
                                str(args.head) + "," + str(args.prop_drop) + "," + str(args.max_span_size) + "," + str(args.seed) + "," + args.model_path + "," + self._log_path + ", ,")

                for i, csv_content in enumerate(csv_reader):
                    if i == 1:
                        txt_file.write(','.join(csv_content[0].split(";")) + "\n")
    ...
    def _load_model(self, input_reader):
        ...
        model = model_class.from_pretrained(self._args.model_path,
                                            ...
                                            head = self._args.head,
                                            freeze_transformer=self._args.freeze_transformer,
                                            cache_dir=self._args.cache_path)

        # model_path = self._args.model_path if self._args.model_path.endswith('.bin') else os.path.join(self._args.model_path, 'pytorch_model.bin')
        # state_dict = torch.load(model_path)  
        # # state_dict = torch.load(model_path, map_location=torch.device('cpu'))  
        # # print("\n\nspert_train.py -> state_dict : \n\n", state_dict)
        # # print(type(state_dict))  
        # #
        #
        # state_dict2 = {"albert." + k : v for k, v in state_dict.items()}

        # state_dict2 = OrderedDict()
        # for k, v in state_dict.items():
        #     state_dict2["albert.bert." + k[7:]] = v

        # print("\n\nspert_train.py -> state_dict : (after adding 'albert.bert.')\n\n", state_dict)
        # print(type(state_dict))  
        #
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # 
            entity_logits, entity_o_logits, rel_logits = model(encodings=batch['encodings'], context_masks=batch['context_masks'],       
                                              entity_masks=batch['entity_masks'], entity_sizes=batch['entity_sizes'],   
                                              relations=batch['rels'], rel_masks=batch['rel_masks'])             
            
            # compute loss and optimize parameters
            batch_loss = compute_loss.compute(entity_s_logits=entity_logits, entity_o_logits=entity_o_logits, rel_logits=rel_logits, 
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                result = model(encodings=batch['encodings'], context_masks=batch['context_masks'],
                               ...
                               inference=True)
                entity_clf_mixed, rel_clf, rels = result


                # evaluate batch
                evaluator.eval_batch(entity_clf_mixed, rel_clf, rels, batch)
        ...
        # average_f1 = rel_eval[2] + rel_nec_eval[2]   

        average_f1 = ner_eval[2] + rel_eval[2] + rel_nec_eval[2]   
        # average_f1 = ner_eval[5] + rel_eval[5] + rel_nec_eval[5]   
